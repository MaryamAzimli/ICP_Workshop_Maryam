...
public func borrowBook(readerId: ReaderId, bookId: BookId): async Bool {
  ...
  public func returnBook(readerId: ReaderId, bookId: BookId): async Bool {
    let userOption = Trie.find(readers, ReaderKey(readerId), Nat32.equal);
    let bookOption = Trie.find(books, BookKey(bookId), Nat32.equal);

    switch (userOption, bookOption) {
        case (?user, ?book) {
            if (List.find(user.borrowed_books_ids, func (id: BookId): Bool { id == bookId }) != null) {
                books := Trie.replace(
                    books,
                    BookKey(bookId),
                    Nat32.equal,
                    ?{ book with avibility = true }
                ).0;
                ...
                readers := Trie.replace(
                    ...
                    ?{
                        user with 
                        num_of_boooks_borrowed = user.num_of_boooks_borrowed - 1;
                        borrowed_books_ids = List.filter(
                            user.borrowed_books_ids,
                            func (id: BookId): Bool { id != bookId }
                        );
                    }
                ).0;
